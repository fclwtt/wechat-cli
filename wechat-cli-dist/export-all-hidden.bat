...
title WeChat Full Export (Hidden)

REM Log file (UTF-8)
set LOG_FILE=%~dp0export-all-log.txt
...
powershell -Command "Add-Content -Path '%LOG_FILE%' -Value '[STEP] Starting full export...' -Encoding UTF8" >nul 2>&1

REM Run full export (all accounts, all chats, full history) - UTF-8 output
"%~dp0wechat-cli.exe" export-all-accounts --output "%OUTPUT_DIR%" 2>&1 | powershell -Command "$input | Add-Content -Path '%LOG_FILE%' -Encoding UTF8"
